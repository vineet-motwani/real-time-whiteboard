...
board.onpointerdown = function (event) {
    ...
    if (event.button == 0 && !lockDrawing && (boardMode == 'pen' || boardMode == 'eraser')) {
        points = [[(event.offsetX - transformX) / transformScale, (event.offsetY - transformY) / transformScale]]
        simplify2Path = appendPath()
        this.setPointerCapture(event.pointerId)
    }
}
board.onpointermove = function (event) {
    if (this.hasPointerCapture(event.pointerId) && !lockDrawing && (boardMode == 'pen' || boardMode == 'eraser')) {
